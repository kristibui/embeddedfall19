...
int main()
{
	...
	int count = 0;
	while (true) {
		int userButton = PushButtonGet(board);
		if (userButton  == 0) {
			for (int i = 0; i < 5; i++) {
				buttonStates[i] = 0;
			}
			...
			// button left shifts the count one to the left
			buttonStates[0] = 1;
			count<<= 1;
			cout << "Count: " << count << endl;
		}
		else if (userButton == 2 && buttonStates[1] != 1) {
			// button right shifts the count one to the left
			buttonStates[1] = 1;
			count>>= 1;
			cout << "Count: " << count << endl;
		}
		else if (userButton == 3 && buttonStates[2] != 1) {
			// button up increments the count by 1
            buttonStates[2] = 1;
            count += 1;
			cout << "Count: " << count << endl;
        }
		else if (userButton == 4 && buttonStates[3] != 1) {
			// button right decrements the count by 1
            buttonStates[3] = 1;
            count -= 1;
			cout << "Count: " << count << endl;
        }
		else if (userButton == 5 && buttonStates[4] != 1) {
			// center button resets the count to what is on the switches
            buttonStates[4] = 1;
            count = ReadAllSwitches(board);
			cout << "Count: " << count << endl;
        }
		WriteAllLeds(board, count);
	}
	delete board;
}
